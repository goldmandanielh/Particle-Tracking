function [sel, selRes, anaRes]=analyzeSingleColorTracks(dataDir, baseName, dataID, showTracks,maskFile)
setTackAnalysisParameter;
load([dataDir baseName num2str(dataID) '.mat'],'-mat');
if nargin<5
    load([dataDir baseName num2str(dataID) '_mask.mat'],'-mat');   %load mask
else
    load(fullfile(dataDir, maskFile),'-mat');
end
tracks=tracksFinal(removeShortTrack(tracksFinal, minTrackLength));   %select track length
